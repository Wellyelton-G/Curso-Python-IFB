...
Este guia explica, passo a passo, como migrar o banco de dados do sistema de biblioteca Django do SQLite para o PostgreSQL, garantindo backup e restauração dos dados sem perdas de informações contidas no banco de dados.
...
3. Crie um usuário e senha para acesso (guarde essa informação para ser utilizada depois).
...
- Abra o arquivo `db.json` no VS Code ou no bloco de notas (notepad). 
- Verifique se está salvo como UTF-8 (rodapé do VS Code). No notepad, quando for salvar use a Codificação UTF-8.
